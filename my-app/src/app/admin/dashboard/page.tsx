...
export default async function AdminDashboardPage() {
  ...
  try {
    const [users, comics, reviews, favorites, pending, comicRows] =
      await Promise.all([
        userService.countUsers(),
        comicService.countComics(),
        reviewService.countReviews(),
        favoriteService.countAll(),
        reportService.countPending(),
        comicService.listComics(),
      ]);
    ...
    recentComics = comicRows.slice(0, 4).map((c) => ({
      id: c.slug,
      title: c.title,
      image: c.coverPhoto,
      type: c.category?.name ?? "Comic",
        ...
        <section>
          <div className="flex flex-col gap-6 border-b border-white/10 pb-6 lg:flex-row lg:items-end lg:justify-between">
            ...
            <div className="grid gap-3 sm:grid-cols-2">
              ...
              <Link
                href="/admin/reviews"
                className="rounded-2xl border border-[#d9ccff]/20 bg-[#11012e]/80 px-4 py-3 transition-colors hover:border-[#f6a1ff]/40 hover:bg-[#11012e]"
              >
          ...
          <section className="mt-8 grid gap-4 md:grid-cols-2 xl:grid-cols-4">
            {dashboardStats.map(({ label, value, hint, icon: Icon }) => (
              ...
              >
                ...
                <p className="mt-4 text-sm font-medium text-[#d9ccff]">
                  {hint}
                </p>
                  ...
                  {recentComics.map((comic) => (
                    ...
                    >
                      ...
                      <div className="space-y-3 p-4">
                        ...
                        <p className="text-sm leading-6 text-white/65 line-clamp-3">
                          {comic.description}
                        </p>
