...
export default function ComicListClient({
  ...
  const normalizeStatus = (value: string) =>
    value.trim().toUpperCase().replace(/\s+/g, "_").replace(/-/g, "_");

  const filteredComics = useMemo(() => {
    const normalizedQuery = query.trim().toLowerCase();
    const normalizedSelectedStatus = normalizeStatus(status);

    return comics.filter((comic) => {
      const matchesCategory = category === "ALL" || comic.type === category;
      const matchesStatus =
        normalizedSelectedStatus === "ALL" ||
        normalizeStatus(comic.status) === normalizedSelectedStatus;
      const matchesSearch =
